...
#define ERR(tag, ...)  do {if (debug_print >= DEBUG_PRINT_ERR)  { printk(tag "[gapspi] " __VA_ARGS__); }} while (0)
#define INFO(tag, ...) do {if (debug_print >= DEBUG_PRINT_INFO) { printk(tag "[gapspi] " __VA_ARGS__); }} while (0)
#define DBG(tag, ...)  do {if (debug_print >= DEBUG_PRINT_DBG)  { printk(tag "[gapspi] " __VA_ARGS__); }} while (0)
